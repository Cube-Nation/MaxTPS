...
class ChunkTicker implements Runnable {
    ...
    ChunkTicker(MaxTPS p, int ticks) {
        queue = new LinkedBlockingQueue<cz.majncraft.maxtps.ChunkRequest>(99999999);
        plugin = p;
    ...
    public void run() {
        try {
            while (true) {
                ...
                cz.majncraft.maxtps.ChunkRequest req = (cz.majncraft.maxtps.ChunkRequest)queue.take();
...
//                    }
                ...
                if (!ch.done) {
                    ...
                    */
                    ...
                    if (!ch.done) {
                        MyPlayerManager mgr = (MyPlayerManager)req.p.server.getWorldServer(req.p.dimension).getPlayerManager();
