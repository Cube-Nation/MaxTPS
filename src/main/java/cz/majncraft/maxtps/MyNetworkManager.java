...
public class MyNetworkManager extends NetworkManager implements Comparator {
    ...
    public MyNetworkManager() {
        super(abort(),"",null, null);
		try {
			net.minecraft.server.Packet51MapChunk.class.getDeclaredField("buildBuffer").setAccessible(true);
...
//        init_burst();
    ...
    private boolean ischunk(Packet packet) {
        return packet instanceof Packet51MapChunk || packet instanceof Packet130UpdateSign || packet instanceof Packet132TileEntityData || packet instanceof Packet52MultiBlockChange || packet instanceof Packet53BlockChange;// || packet instanceof Packet24MobSpawn || packet instanceof Packet21PickupSpawn || packet instanceof Packet23VehicleSpawn;
    }
...
//        pl.debug("obfuscated! "+count);
    ...
    void compress(Packet51MapChunk pkt) {
        if (pl.oreditor > 0 && p.dimension == 0 && (pkt.e)) obfuscate(pkt.buildBuffer);
        int dataSize = pkt.buildBuffer.length;
        if (deflateBuffer.length < dataSize + 100)
            deflateBuffer = new byte[dataSize + 100];
        deflater.reset();
        deflater.setLevel(1);
        deflater.setInput(pkt.buildBuffer);
        deflater.finish();
...
//                                    chunkpkt = null;
    ...
    public void socket_read() {
        try {
            ...
            while (!killthreads) {
                Packet pkt = Packet.a(this.dinput, this.handler.a());
